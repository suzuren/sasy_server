...
local function loadHuoDongTimeConfig()
	local sql = "SELECT `Index`,Tips,ActivityType,UNIX_TIMESTAMP(StartTime) as StartTime,UNIX_TIMESTAMP(EndTime) as EndTime FROM `sstreasuredb`.`t_huo_dong_time_config`"
...
local function loadRewardGoldFishConfig()
	local sql = "SELECT * FROM `sstreasuredb`.`t_table_reward_gold_fish`"
	local dbConn = addressResolver.getMysqlConnection()
	local rows = skynet.call(dbConn,"lua","query",sql)
	if type(rows)=="table" then
		for _, row in pairs(rows) do
...
local function loadDropGemConfig()
	local sql = "SELECT * FROM `sstreasuredb`.`t_table_drop_gem`"
	local dbConn = addressResolver.getMysqlConnection()
	local rows = skynet.call(dbConn,"lua","query",sql)
	if type(rows)=="table" then
		for _, row in pairs(rows) do
...
local function loadFlyBirdRunMonsterConfig()
	local sql = "SELECT * FROM `sstreasuredb`.`t_table_fly_bird_run_monster`"
	local dbConn = addressResolver.getMysqlConnection()
	local rows = skynet.call(dbConn,"lua","query",sql)
	if type(rows) == "table" then
		for _, row in pairs(rows) do 
